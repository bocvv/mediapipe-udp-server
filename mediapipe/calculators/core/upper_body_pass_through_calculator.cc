...
#include "mediapipe/framework/formats/wrapper_upper_body.pb.h"
...
constexpr char kDetectionsTag[] = "DETECTION";
...
// ignored.
class UpperBodyPassThroughCalculator : public CalculatorBase {
 public:
  ...
  ::mediapipe::Status Process(CalculatorContext* cc) final {
    ...
    WrapperUpperBody* wrapper = new WrapperUpperBody();
    ...
    for (CollectionItemId id = cc->Inputs().BeginId();
      ...
      if (!cc->Inputs().Get(id).IsEmpty()) {
        ...
        if (cc->Inputs().Get(id).Name() == "pose_landmarks_smoothed") {
        ...
        if (cc->Inputs().Get(id).Name() == "pose_detection") {
          // Palm is detected once, not continuously — when it first shows up in the image
            const auto& detection = cc->Inputs().Tag(kDetectionsTag).Get<Detection>();
...
REGISTER_CALCULATOR(UpperBodyPassThroughCalculator);
