...
#include "mediapipe/framework/formats/wrapper_object_tracking.pb.h"
...
// ignored.
class ObjectTrackingPassThroughCalculator : public CalculatorBase {
 public:
  ...
  ::mediapipe::Status Process(CalculatorContext* cc) final {
    ...
    WrapperObjectTracking* wrapper = new WrapperObjectTracking();
    ...
    for (CollectionItemId id = cc->Inputs().BeginId();
      ...
      if (!cc->Inputs().Get(id).IsEmpty()) {
        ...
        /*-------------------------------------------------------------------*/
        if (cc->Inputs().Get(id).Name() == "tracked_detections"){
          ...
          for (int i = 0; i < detections.size(); ++i) {
              ...
              wrapper->mutable_detections()->add_detection();
              int size = wrapper->mutable_detections()->detection_size() - 1;
              wrapper->mutable_detections()->mutable_detection(size)->add_label(detection.label()[0]);
              wrapper->mutable_detections()->mutable_detection(size)->add_score(detection.score()[0]);
              wrapper->mutable_detections()->mutable_detection(size)->mutable_location_data()->mutable_relative_bounding_box()
                ->set_xmin(detection.location_data().relative_bounding_box().xmin());
              wrapper->mutable_detections()->mutable_detection(size)->mutable_location_data()->mutable_relative_bounding_box()
                ->set_ymin(detection.location_data().relative_bounding_box().xmin());
              wrapper->mutable_detections()->mutable_detection(size)->mutable_location_data()->mutable_relative_bounding_box()
                ->set_width(detection.location_data().relative_bounding_box().width());
              wrapper->mutable_detections()->mutable_detection(size)->mutable_location_data()->mutable_relative_bounding_box()
                ->set_height(detection.location_data().relative_bounding_box().height());
          }
        }
      /*-------------------------------------------------------------------*/
...
REGISTER_CALCULATOR(ObjectTrackingPassThroughCalculator);
